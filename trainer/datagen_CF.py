...
class Dataset(object):
    ...
    def sample_data(self, batch_size):
        ...
        if self.dang_count > 0:
            ...
            s_dang, u_dang, s_next_dang, e_dang = np.array([], dtype=np.float32).reshape(0, n_state), np.array([],
                                                                                                               dtype=np.float32).reshape(
                0, m_control), np.array([], dtype=np.float32).reshape(0, n_state), np.array([],
                                                                                            dtype=np.float32).reshape(0,
                                                                                                                      n_state)
        ...
        if self.mid_count > 0:
            ...
            s_mid, u_mid, s_next_mid, e_mid = np.array([], dtype=np.float32).reshape(0, n_state), np.array([],
                                                                                                           dtype=np.float32).reshape(
                0, m_control), np.array([], dtype=np.float32).reshape(0, n_state), np.array([],
                                                                                            dtype=np.float32).reshape(0,
                                                                                                                      n_state)
        ...
        if self.safe_count > 0:
            ...
            s_safe, u_safe, s_next_safe, e_safe = np.array([], dtype=np.float32).reshape(0, n_state), np.array([],
                                                                                                               dtype=np.float32).reshape(
                0, m_control), np.array([], dtype=np.float32).reshape(0, n_state), np.array([],
                                                                                            dtype=np.float32).reshape(0,
                                                                                                                      n_state)
