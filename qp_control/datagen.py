...
class Dataset_with_Grad(object):
    ...
    def add_data(self, grad_h, state, u, u_nominal, state_next):
        """
        args:
            state (n_state,): state of the agent
            obstacle (k_obstacle, n_state): K obstacles
        ...
        max_alpha = torch.max(alpha)
        # print(max_alpha)
        # data = [torch.copy(grad_h).astype(np.float32), torch.copy(state).astype(np.float32), 
        #         torch.copy(u_nominal).astype(np.float32), torch.copy(state_next).astype(np.float32),
        #         torch.copy(state_error).astype(np.float32)]
        data = [grad_h, state, u, u_nominal, state_next]
        # print(min_dist)
    ...
    def sample_data(self, batch_size):
        ...
        if self.dang_count>0:
            grad_dang, s_dang, u_NN_dang, u_dang, s_next_dang = self.sample_data_from_buffer(num_dang, self.buffer_dang)
        else:
            if self.safe_count>0:
                num_safe = num_safe + num_dang
            grad_dang, s_dang, u_NN_dang, u_dang, s_next_dang = torch.zeros(0, self.n_state),torch.zeros(0, self.n_state).reshape(0,n_state),torch.zeros(0,self.m_control),np.array([],dtype = np.float32).reshape(0,m_control),torch.zeros(0, self.n_state).reshape(0,n_state)

        if self.mid_count>0:
            grad_mid, s_mid, u_NN_mid, u_mid, s_next_mid = self.sample_data_from_buffer(num_mid, self.buffer_mid)
        else:
            if self.safe_count>0:
                num_safe = num_safe + num_mid
            grad_mid, s_mid, u_NN_mid, u_mid, s_next_mid = torch.zeros(0, self.n_state),torch.zeros(0, self.n_state).reshape(0,n_state),torch.zeros(0,self.m_control),np.array([],dtype = np.float32).reshape(0,m_control),torch.zeros(0, self.n_state).reshape(0,n_state)

        # print(self.dang_count)
        # print(self.mid_count)
        # print(self.safe_count)
        if self.safe_count>0:
            grad_safe, s_safe, u_NN_safe, u_safe, s_next_safe = self.sample_data_from_buffer(num_safe, self.buffer_safe)
            # print(s_safe.shape)
        else:
            grad_safe, s_safe, u_NN_safe, u_safe, s_next_safe = torch.zeros(0, self.n_state),torch.zeros(0, self.n_state).reshape(0,n_state),torch.zeros(0,self.m_control),np.array([],dtype = np.float32).reshape(0,m_control),torch.zeros(0, self.n_state).reshape(0,n_state)

        grad_h = torch.cat([grad_safe,grad_dang,grad_mid], axis = 0)
        s = torch.cat([s_safe, s_dang, s_mid], axis=0)
        u_NN = torch.cat([u_NN_safe, u_NN_dang, u_NN_mid], axis=0)
        u = np.concatenate([u_safe, u_dang, u_mid], axis=0)
        s_next = torch.cat([s_next_safe, s_next_dang, s_next_mid], axis=0)

        return grad_h, s, u_NN, u, s_next

    def sample_data_from_buffer(self, batch_size, buffer):
        indices = np.random.randint(len(buffer), size=(batch_size))
        grad_h = torch.zeros(batch_size,self.n_state)
        s = torch.zeros(batch_size, self.n_state)
        u_NN = torch.zeros(batch_size, self.m_control)
        u = np.zeros((batch_size, self.m_control), dtype=np.float32)
        s_next = torch.zeros(batch_size, self.n_state)
        for i, ind in enumerate(indices):
            grad, state, u_neural, u_nominal, state_next = buffer[ind]
            grad_h[i] = grad
            s[i] = state
            u_NN[i] = u_neural
            u[i] = u_nominal
